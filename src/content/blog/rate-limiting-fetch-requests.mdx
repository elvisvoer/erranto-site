...
reaching an API rate limit. I used to write similar code in the past so the
...
of the series proposes an initial "not production ready" solution:
...
=== 1` is `true` and we invoke the `#loop` method, we pop the item from the
...
processing the items. This behavior is closer to what we need, if you remove the
`await` in front of the `limiter.fetch` call you will notice that all requests
...
of the series goes in a direction I don't quite agree with. Instead, I will
...
If you are executing the code above you will notice that only two requests are
...
executed sequentially. To better visualize this use the following example code:
...
and, let's also mock global `fetch` for now to use longer requests times.
Add this function above the `HTTPLimiter` class:
...
It returns a `Promise` that looks like a `Response` (has a `json` method which
returns a `Promise`) which resolves after 300ms. Running the code should output
...
That's around 8ish seconds instead of around 5. Not good. Let's now fix this
by making our loop parallel. The following code should show better results:
...
Indeed, this solution yields better results. Our requests are now executed in
parallel after the loop awaits for the delay.
...
With just a few little fixes to the original code, we reached our goal of having
a fully functional API-rate-limiter implementation.
